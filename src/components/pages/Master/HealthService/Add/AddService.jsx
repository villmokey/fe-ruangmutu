...
const FormAddService = ({
  ...
  const handleCreate = () => {
    fetchApiPost("/service-unit", accessToken, {
      name: payload.name,
    })
      .then((res) => {
        if (res && res.code === 200) {
          message.success("Berhasil menambahkan unit layanan kesehatan");
          if (onSuccess) {
            onSuccess();
          }
          if (onClose) {
  ...
  const handleUpdate = () => {
    fetchApiPut("/service-unit/" + payload.id, accessToken, {
      name: payload.name,
    })
      .then((res) => {
        if (res && res.code === 200) {
          message.success("Berhasil mengubah unit layanan kesehatan");
          if (onSuccess) {
            onSuccess();
          }
          if (onClose) {
