...
const FormAdd = ({
  ...
  const handleCreate = () => {
    fetchApiPost("/program", accessToken, {
      name: payload.name,
      color: payload.color,
    })
      .then((res) => {
        if (res && res.code === 200) {
          message.success("Berhasil menambahkan program mutu");
          if (onSuccess) {
            onSuccess();
          }
          if (onClose) {
  ...
  const handleUpdate = () => {
    ...
    fetchApiPut("/program/" + payload.id, accessToken, {
      color: payload.color,
      name: payload.name,
    })
      .then((res) => {
        if (res && res.code === 200) {
          message.success("Berhasil mengubah program mutu");
          if (onSuccess) {
            onSuccess();
          }
          if (onClose) {
  ...
  return (
    ...
    >
      <Title level={4}>{isCreate ? "TAMBAH" : "EDIT"} PROGRAM MUTU</Title>
      <Form layout={"vertical"}>
        ...
        <Textfield
          ...
          required
          value={payload.color}
          onChange={(e) => payloadSetter({ ...payload, color: e.target.value })}
        />
